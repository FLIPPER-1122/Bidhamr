...
## Ufravigelige regler
1. **Claude pusher ALDRIG til git.** Filip pusher og merger selv. Arbejd på en separat branch pr. opgave (fx `agent/fase1-betalingsmodel`).
...
3. **Opret branch**: `agent/<kort-navn>`.
4. **Uddeleger** til den rigtige agent – kod ikke selv, hvis en agent kan gøre det:
...
5. Giv hver agent en klar opgave: hvad, hvilke filer, hvilke regler fra beslutningerne, og hvad den skal melde tilbage.
6. **Altid reviewer bagefter**, når en opgave rører database, betaling eller brugeradgang. Retter du ikke de fejl, reviewer finder, er opgaven ikke færdig.
7. **Tester** til sidst på alt, brugeren kan se eller bruge.
8. **Meld tilbage til Filip** på dansk, kort:
   - hvad der er lavet
   - hvilken branch det ligger på
